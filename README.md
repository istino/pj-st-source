...
**4인 팀 프로젝트** (개발 4) · 팀 리더 **김민철** · 2024.11 – 2025.01 · 최종 프로젝트 23팀 중 1위

| | |
|---|---|
| 김민철 담당 | 스탯/버프·디버프 시스템(Query Object), 업적 시스템, 패시브 스킬 확장, 브랜치 통합 |
| 공개 범위 | **코드 공개판이라 원본 저장소의 커밋 이력은 포함하지 않는다** (단일 커밋으로 정리). 아트 에셋 제외 |

팀 구성원별 담당 범위는 문서 하단 「팀 소개」에 있다.
...
> ### ⚠️ 이 저장소는 코드 공개용입니다
> 작성한 스크립트와 씬·프리팹 구성만 담고 있으며, **원작에서 비롯된 모델·애니메이션·텍스처 등 아트 에셋은 포함하지 않습니다.**
> 따라서 클론해도 그대로 실행되지 않습니다. 실행 가능한 빌드는 상단 시연 영상으로 확인해 주세요.
> 아래 이미지는 개발 당시의 화면을 기록한 스크린샷입니다.
